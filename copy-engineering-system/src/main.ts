...
import type { CopyBrief, FinalCopyResult } from "./types.js";
...
const DEFAULT_BRIEF: CopyBrief = {
  ...
  offer:
    "In-home personal care services for seniors — founded and led by Shannon Stafford Simpson, a Certified Nursing Assistant. The Shannon Standard: free same-day assessment, in-home evaluation within 24 hours, caregiver match within 48 hours, supervised first visit, 72-hour follow-up.",
  audience:
    "Adult daughters aged 45-62 managing a parent's care from near or far. At the kitchen table at 10:45pm. Something happened today. They need permission to stop carrying this alone.",
...
// ---------------------------------------------------------------------------
function parseBriefFromArgs(): CopyBrief {
  const args = process.argv.slice(2);
  const briefArg = args.find((a) => a.startsWith("--brief="));

  if (!briefArg) {
    process.stderr.write(
      "[main] No --brief argument provided. Using default sample brief.\n"
    );
  ...
  return JSON.parse(raw) as CopyBrief;
}

// ---------------------------------------------------------------------------
// Output formatting
// ---------------------------------------------------------------------------
function formatOutput(result: FinalCopyResult): string {
  ...
  const violations =
    !fabricationAudit?.passed && fabricationAudit?.violations?.length
      ? fabricationAudit.violations.map((v) => `  - ${v}`).join("\n")
      : "";

  const scorecardTable = formatScorecardTable(
    result.scorecard,
    result.agentResults
  );
...
// Save result JSON
// ---------------------------------------------------------------------------
function saveResultJson(result: FinalCopyResult): string {
...
async function main(): Promise<void> {
  ...
  process.stderr.write(
    `[main] Starting Copy Engineering System for: ${brief.businessName}\n`
  );
  process.stderr.write(
    `[main] Asset: ${brief.assetType} | Target behavior: ${brief.desiredBehavior}\n\n`
  );
