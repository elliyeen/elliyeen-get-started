...
import {
  ...
  PASS_THRESHOLD_STAGE,
} from "../scoring/scorecard.js";
import type { AgentResult, AgentScore, Scorecard } from "../types.js";

// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeAgentResult(name: string, score: number): AgentResult {
  const raw: AgentScore = {
    score,
...
function makeAllAgentResults(scores: Record<string, number>): AgentResult[] {
  return Object.entries(scores).map(([name, score]) =>
    makeAgentResult(name, score)
  );
}
...
describe("buildScorecard", () => {
  ...
  it("includes all 10 stage scores in output", () => {
    const results = makeAllAgentResults(
      Object.fromEntries(ALL_STAGES.map((s) => [s, 7]))
    );
    const scorecard = buildScorecard(results);
  ...
  it("handles all zeros gracefully", () => {
    const results = makeAllAgentResults(
      Object.fromEntries(ALL_STAGES.map((s) => [s, 0]))
    );
    const scorecard = buildScorecard(results);
  ...
  it("handles perfect scores", () => {
    const results = makeAllAgentResults(
      Object.fromEntries(ALL_STAGES.map((s) => [s, 10]))
    );
    const scorecard = buildScorecard(results);
...
describe("getWeakestStage", () => {
  ...
  it("returns attention when all stages tie", () => {
    ...
    assert.ok(ALL_STAGES.includes(result as typeof ALL_STAGES[number]));
  });

  it("handles advocacy as weakest", () => {
    const scores = Object.fromEntries(ALL_STAGES.map((s) => [s, 8]));
    scores["advocacy"] = 2;
    const scorecard: Scorecard = { ...scores, overall: 7.4 } as Scorecard;
...
describe("getStrongestStage", () => {
  ...
  it("handles action as strongest", () => {
    ...
    scores["action"] = 10;
    const scorecard: Scorecard = { ...scores, overall: 5.5 } as Scorecard;
...
describe("formatScorecardTable", () => {
  ...
  it("includes all 10 stage names in the table", () => {
    ...
    for (const stage of ALL_STAGES) {
      ...
      assert.ok(
        ...
        `Table should include stage label: ${label}`
      );
    }
  });
  ...
  it("includes FAIL when overall < 6.5", () => {
    const results = makeAllAgentResults(
      Object.fromEntries(ALL_STAGES.map((s) => [s, 5]))
    );
    const scorecard = buildScorecard(results);
    const table = formatScorecardTable(scorecard, results);
...
describe("getFailingStages", () => {
  ...
  it("returns all stages when all scores are 0", () => {
    const scorecard: Scorecard = Object.fromEntries(
      [...ALL_STAGES.map((s) => [s, 0]), ["overall", 0]]
    ) as Scorecard;
    const failing = getFailingStages(scorecard);
