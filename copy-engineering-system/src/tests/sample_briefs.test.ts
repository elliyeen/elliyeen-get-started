...
import type { CopyBrief, FinalCopyResult, AgentScore, AuditResult, BehaviorAuditResult } from "../types.js";
import { runConductor } from "../conductor.js";

// ---------------------------------------------------------------------------
// Deterministic mock responses
// ---------------------------------------------------------------------------
function makeMockAgentScore(overrides: Partial<AgentScore> = {}): string {
...
function makeMockFabricationAudit(): string {
  return JSON.stringify({
    passed: true,
    violations: [],
...
function makeMockBehaviorAudit(): string {
  return JSON.stringify({
    overallScore: 7,
...
function createMockClient(): Anthropic {
  ...
  (client.messages as any).create = async (): Promise<Anthropic.Message> => {
    callCount++;
...
// 8 Sample Briefs
// ---------------------------------------------------------------------------
const briefs: { name: string; brief: CopyBrief }[] = [
  {
    ...
    brief: {
      ...
      offer:
        "In-home personal care services for seniors, founded by Shannon Stafford Simpson (CNA). The Shannon Standard: same-day assessment, 24hr in-home evaluation, 48hr caregiver match, supervised first visit, 72hr written follow-up.",
      audience:
        "Adult daughters aged 45-62 managing a parent's care. Kitchen table, 10:45pm. Something happened today.",
  ...
  },
  {
    ...
    brief: {
      ...
      offer:
        "AI-driven revenue engineering system that identifies untapped growth levers in mid-market B2B companies within 30 days.",
      audience:
        "B2B founders and revenue leaders at $5M-$50M companies who have hit a growth plateau and suspect the answer is in their data.",
  ...
  },
  {
    ...
    brief: {
      ...
      offer:
        "Athlete-specific financial legacy planning. Protects career earnings, structures generational wealth, and prevents the post-career financial collapse that affects 78% of professional athletes within 5 years of retirement.",
      audience:
        "Professional athletes aged 22-35 with first major contract. Their financial life is being managed by someone else. They feel the clock ticking.",
      desiredBehavior: "appointment",
      assetType: "landing page",
      awarenessLevel: "problem_aware",
  ...
  },
  {
    ...
    brief: {
      ...
      offer:
        "12-week online golf improvement program. Guaranteed to lower your handicap by 5 strokes or full refund. Taught by PGA instructor with 20 years experience.",
      audience:
        "Amateur golfers aged 35-60 who have been stuck at the same handicap for 2+ years despite regular play. They have disposable income and take the sport seriously.",
  ...
  },
  {
    ...
    brief: {
      ...
      offer:
        "Flowdash eliminates manual handoffs between sales, ops, and CS. Most companies lose 23% of revenue to internal friction they can't see.",
      audience:
        "B2B SaaS operators and COOs on LinkedIn. They know they have a process problem. They don't know a product exists that solves it.",
  ...
  },
  {
    ...
    brief: {
      ...
      offer:
        "90-day organizational effectiveness audit that identifies the single highest-leverage change a company can make to their operating model.",
      audience:
        "CEOs of 50-200 person companies who have grown fast and feel the organization starting to groan under its own weight. Too many meetings. Decisions slowing down.",
      desiredBehavior: "reply",
  ...
  },
  {
    ...
    brief: {
      ...
      offer:
        "Fund full scholarships for first-generation college students from underserved communities in Houston. $2,400 = one full year of tuition for one student.",
      audience:
        "Existing donors and warm prospects who believe in education equity. They have given before or expressed interest. They want their money to land somewhere specific.",
  ...
  },
  {
    ...
    brief: {
      ...
      offer:
        "New patient exam and cleaning — $89 flat fee, no hidden costs. Includes digital X-rays, full exam, professional cleaning, and personalized care plan.",
      audience:
        "Adults 25-55 who have been putting off going to the dentist for 1-3 years. They are slightly anxious, don't have a current dentist, and want it to be easy and affordable.",
      desiredBehavior: "appointment",
...
describe("Sample Briefs — Copy Engineering System", () => {
  for (const { name, brief } of briefs) {
    it(`should process brief: ${name}`, async () => {
      ...
      const result: FinalCopyResult = await runConductor(brief, mockClient);

      // 1. Target behavior is correctly set
      assert.equal(
        result.targetBehavior,
        brief.desiredBehavior,
        `targetBehavior should match brief.desiredBehavior`
      );
      ...
      assert.equal(
        result.assetType,
        brief.assetType,
        `assetType should match brief.assetType`
      );
      ...
      assert.equal(
        fabricationAudit.passed,
        true,
        "fabrication audit should pass"
      );
      assert.equal(
        fabricationAudit.violations.length,
        0,
        "fabrication audit should have no violations"
      );
      ...
      for (const stage of stages) {
        assert.ok(
          typeof scorecard[stage] === "number",
          `scorecard.${stage} should be a number`
        );
        assert.ok(
          scorecard[stage] >= 1 && scorecard[stage] <= 10,
          `scorecard.${stage} should be between 1 and 10`
        );
      }
      assert.ok(
        typeof scorecard.overall === "number",
        "scorecard.overall should be a number"
      );

      // 6. Weakest and strongest stages are identified
      assert.ok(
        result.weakestStage && result.weakestStage.length > 0,
        "weakestStage should be identified"
      );
      assert.ok(
        result.strongestStage && result.strongestStage.length > 0,
        "strongestStage should be identified"
      );
      assert.ok(
        stages.includes(result.weakestStage as typeof stages[number]),
        `weakestStage "${result.weakestStage}" should be a valid stage name`
      );
      ...
      assert.equal(
        ...
        10,
        "should have results for all 10 agents"
      );

      // 8. Iterations used is within bounds
      assert.ok(
        result.iterationsUsed >= 1 && result.iterationsUsed <= 5,
        "iterationsUsed should be between 1 and 5"
      );
    });
  }
});
