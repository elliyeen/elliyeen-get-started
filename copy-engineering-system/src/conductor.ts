 ...
 */
function validateBrief(brief: CopyBrief): void {
  const required: (keyof CopyBrief)[] = [
    "businessName",
    "industry",
    "offer",
    "audience",
  ...
  if (missing.length > 0) {
    throw new Error(
      `CopyBrief is missing required fields: ${missing.join(", ")}`
    );
  }
}
...
async function runAllAgents(
  ...
  const toAgentResult = (name: string, raw: AgentScore): AgentResult => ({
    agentName: name,
    score: raw.score,
    notes: [raw.reason],
...
function logIteration(
  ...
  process.stderr.write(
    `[conductor] Iteration ${iteration} — overall:${overall}  ${scores}  ${passes ? "PASS" : "FAIL"}\n`
  );
}
...
export async function runConductor(
  ...
  for (let i = 1; i <= MAX_ITERATIONS; i++) {
    ...
    if (!fabricationPassed) {
      process.stderr.write(
        `[conductor] Fabrication violations: ${fabricationAudit.violations.join(" | ")}\n`
      );
    }
    ...
    if (behaviorAudit.mustFix.length > 0) {
      process.stderr.write(
        `[conductor] Behavior audit must-fix: ${behaviorAudit.mustFix.slice(0, 2).join(" | ")}\n`
      );
    }

    if (passes) {
      process.stderr.write(
        `[conductor] Copy passed all thresholds at iteration ${i}.\n`
      );
    ...
    if (i < MAX_ITERATIONS) {
      process.stderr.write(
        `[conductor] Rewriting copy with agent feedback...\n`
      );
      ...
      currentCopy = await rewriteWithFeedback(
        brief,
        currentCopy,
        agentResults,
        i,
  ...
  process.stderr.write(
    `[conductor] Max iterations (${MAX_ITERATIONS}) reached. Returning best result.\n`
  );
