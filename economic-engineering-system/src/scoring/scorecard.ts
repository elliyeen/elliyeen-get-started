...
export function rankOpportunities(
  ...
  const allRecommendations: AgentRecommendation[] = agentOutputs.flatMap(
    (output) => output.recommendations
  );

  const scored = allRecommendations.map((rec) => {
    const components: ScoreComponents = rec.score_components ?? {
      profit_impact: 0,
      customer_value: 0,
...
export function formatOpportunityTable(opportunities: RankedOpportunity[]): string {
...
export function computeOverallConfidence(agentOutputs: AgentOutput[]): number {
