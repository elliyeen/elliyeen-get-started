...
export async function runOperationsAgent(
  ...
  client: Anthropic
): Promise<AgentOutput & { waste_identified: WasteItem[] }> {
  const userMessage = `
BUSINESS PROFILE:
...
Return valid JSON only — no prose outside the JSON block.
  ...
  const parsed = parseJsonFromResponse<OperationsRaw>(text);
