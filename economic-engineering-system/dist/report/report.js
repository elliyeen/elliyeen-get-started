...
function formatOpportunitySection(opps) {
    return opps
        .slice(0, 3)
        .map((opp) => {
...
export function generateMarkdownReport(report) {
    ...
    const wasteTable = wasteRows.length > 0
        ? formatTable(["Waste Type", "Description", "Est. Annual Cost", "Confidence"], wasteRows)
    ...
    const qualityTable = qualityRows.length > 0
        ? formatTable(["Risk", "Customer Impact", "Financial Impact", "Prevention Action"], qualityRows)
    ...
    const missingTable = missingRows.length > 0
        ? formatTable(["Data Needed", "Why It Matters", "How to Get It", "Priority"], missingRows)
    ...
    const roadmap30Table = roadmap30Rows.length > 0
    ...
    const roadmap90Table = roadmap90Rows.length > 0
...
**Estimated Cost to Run:** ${exp.estimated_cost_to_run}`
    ...
    // Assumptions log
    const assumptionsLog = report.assumptions_log.length > 0
