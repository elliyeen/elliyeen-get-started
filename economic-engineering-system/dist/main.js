...
function parseBriefFromArgs() {
    ...
    if (!briefArg) {
        process.stderr.write(`[main] No --brief argument provided. Using default: ${resolved}\n`);
    }
...
function logSummary(result) {
    ...
    process.stderr.write(`Overall Confidence: ${result.analysis_confidence.toFixed(2)}\n`);
    process.stderr.write("===========================\n\n");
}
// ---------------------------------------------------------------------------
// Main
...
async function main() {
    ...
    process.stderr.write(`[main] Economic Engineering System — ${profile.business_name}\n`);
    process.stderr.write(`[main] Industry: ${profile.industry} | Goal: ${profile.current_goal}\n\n`);
