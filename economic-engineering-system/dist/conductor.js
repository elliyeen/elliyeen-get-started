...
import { rankOpportunities, computeOverallConfidence, } from "./scoring/scorecard.js";
...
function validateProfile(profile) {
    ...
    if (missing.length > 0) {
        throw new Error(`BusinessProfile is missing required fields: ${missing.join(", ")}`);
    }
}
// ---------------------------------------------------------------------------
// Run all 8 primary agents in parallel (experiment agent runs after ranking)
// ---------------------------------------------------------------------------
async function runPrimaryAgents(profile, client) {
    process.stderr.write("[conductor] Running 8 primary agents in parallel...\n");
    const [demand, customer, value_stream, constraint, profit, operations, quality, capital] = await Promise.all([
...
// ---------------------------------------------------------------------------
async function generateExecutiveSummary(report, client) {
    const context = `
...
// ---------------------------------------------------------------------------
function buildRoadmap30(opportunities, constraint) {
    const rows = [];
    rows.push({
        week: "Week 1",
...
function buildRoadmap90(opportunities, constraint) {
    const rows = [];
    rows.push({
        month: "Month 1",
    ...
    rows.push({
        ...
        expected_output: "2–3 validated improvements compounding. Margin impact measurable. Next constraint identified.",
    });
    return rows;
}
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
function aggregateMissingData(agentOutputs) {
    const seen = new Set();
    const items = [];
...
// ---------------------------------------------------------------------------
function aggregateAssumptions(agentOutputs) {
    const seen = new Set();
    const all = [];
    agentOutputs.forEach((output) => {
...
// Main conductor
// ---------------------------------------------------------------------------
export async function runConductor(profile, client) {
    validateProfile(profile);
    process.stderr.write(`[conductor] Starting Economic Engineering System for: ${profile.business_name}\n`);
    // Step 1 — Run 8 primary agents in parallel
    const { demand, customer, value_stream, constraint, profit, operations, quality, capital } = await runPrimaryAgents(profile, client);
    ...
    process.stderr.write(`[conductor] ${rankedOpportunities.length} opportunities ranked. Top score: ${rankedOpportunities[0]?.opportunity_score ?? 0}/100\n`);
    // Step 3 — Run experiment agent against top-ranked opportunity
    const topOpportunity = rankedOpportunities[0]?.recommendation;
    let experimentOutput = null;
    if (topOpportunity) {
    ...
    const allOutputs = experimentOutput
        ? [...primaryOutputs, experimentOutput]
        : primaryOutputs;
    // Step 4 — Aggregate waste and quality risks
    const wasteFound = operations.waste_identified ?? [];
    const qualityRisks = quality.quality_risks ?? [];
    ...
    const report = {
        ...
        current_state: {
            ...
            estimated_gross_margin: profile.financials?.gross_margin_percent
                ...
                : "[MISSING]",
            primary_revenue_driver: profile.revenue_streams?.[0]?.name ?? profile.primary_offer,
        ...
        recommended_first_experiment: experimentOutput?.recommended_first_experiment ?? null,
    ...
    process.stderr.write(`[conductor] Report complete. Overall confidence: ${analysisConfidence.toFixed(2)}\n`);
    return report;
}
